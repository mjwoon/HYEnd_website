...
import { announcementService, AnnouncementResponse, AnnouncementComment } from '@/services/announcementService';
...
export default function NoticeDetailPage() {
  ...
  const [post, setPost] = useState<AnnouncementResponse | null>(null);
  ...
  useEffect(() => {
    if (!id) return;
    announcementService.getDetail(Number(id)).then((p) => {
      setPost(p);
      setLoading(false);
    }).catch(() => setLoading(false));
    announcementService.getComments(Number(id)).then(setComments);
  }, [id]);

  const handleAddComment = async () => {
    if (!commentText.trim() || !id) return;
    const added = await announcementService.addComment(Number(id), {
  ...
  const handleDeleteComment = async (commentId: number) => {
    await announcementService.removeComment(commentId);
    setComments((prev) => prev.filter((c) => c.id !== commentId));
  };

  const handleDeletePost = async () => {
    if (!id || !confirm('공지를 삭제하시겠습니까?')) return;
  ...
  if (!post) return <Wrapper><Card><EmptyText>공지를 찾을 수 없습니다.</EmptyText></Card></Wrapper>;

  return (
    <Wrapper>
      <Card>
        <PageHeader>
          <BackButton onClick={() => navigate('/board/notice')}>← 공지사항</BackButton>
          {isAdmin && <DeletePostBtn onClick={handleDeletePost}>삭제</DeletePostBtn>}
        </PageHeader>

        {post.isImportant && <PinBadge>📌 고정 공지</PinBadge>}
...
const Wrapper = styled.div`
    ...
    padding: 60px 40px; min-height: calc(100vh - 64px);
...
const BackButton = styled.button`
    background: none; border: none; color: ${({ theme }) => theme.colors.text.secondary};
...
const PinBadge = styled.span`
    ...
    font-size: 0.8125rem; color: ${({ theme }) => theme.colors.neonGreen};
`;

const PostTitle = styled.h1`
    font-size: 1.5rem; font-weight: ${({ theme }) => theme.typography.fontWeight.black};
    color: ${({ theme }) => theme.colors.text.primary}; margin: 0; line-height: 1.4;
`;
...
const PostContent = styled.p`
    ...
    line-height: 1.85; margin: 0; white-space: pre-wrap;
`;
...
`;
const CommentDate = styled.span`font-size: 0.75rem; color: ${({ theme }) => theme.colors.text.secondary};`;
...
const CommentDeleteBtn = styled.button`
    ...
    padding: 2px 8px; font-size: 0.75rem; cursor: pointer; font-family: inherit; flex-shrink: 0;
...
const EmptyText = styled.p`text-align: center; color: ${({ theme }) => theme.colors.text.secondary}; margin: 0;`;
...
const LightboxImg = styled.img`max-width: 90vw; max-height: 90vh; border-radius: 8px; cursor: default;`;
