...
19-1. [별도 전달 항목 (안전한 채널로)](#19-1-별도-전달-항목-안전한-채널로)
...
- [x] ~~**구 서버 정리:**~~ 완료. 이전 AWS 계정의 기존 서버(`13.209.76.52`) 정리 완료.
- [x] ~~**`.env`의 `VITE_API_URL` 중복 정리:**~~ 완료.
...
## 19-1. 별도 전달 항목 (안전한 채널로)

아래 항목들은 **실제 접근 권한/시크릿**이므로 이 문서에 값을 적지 않는다.
반드시 **비밀번호 관리자(1Password/Bitwarden) 공유, 대면 전달, 또는 암호화 채널**로 넘긴다.
이메일·메신저 평문 전송 금지.

| 항목 | 내용 | 없으면 안 되는 이유 |
|---|---|---|
| **SSH 키 파일** | `hyend-new-key.pem` | 이게 없으면 서버 접속 자체가 불가능 (가장 중요) |
| **AWS 콘솔 접근** | 계정 `126052242187` 로그인, 또는 신규 담당자를 IAM 사용자로 추가 | EC2·S3·보안그룹·Elastic IP 관리 |
| **hosting.kr 계정** | 도메인(hyend.kr) 관리 로그인 | DNS 레코드·도메인 갱신 관리 |
| **GitHub 저장소 권한** | `mjwoon/HYEnd_website` collaborator 추가 | 코드 push·Secrets 관리·배포 |
| **OpenAI 대시보드** | API 키 발급/조회 계정 | AI 기능 키 재발급 시 |
| **LiveKit 대시보드** | 프로젝트 API 키 계정 | 화상회의 키 재발급 시 |

### `.env` / Secrets 실제 값 확인 방법

값을 문서에 옮기지 말고, 권한을 받은 담당자가 **직접 조회**한다.

- **서버 `.env` 실제 값:** SSH 접속 후 확인
  ```bash
  cat ~/HYEnd_website/.env
  ```
  (SSH 키만 안전하게 전달받으면 모든 운영 시크릿을 여기서 볼 수 있다.)
- **GitHub Secrets:** 보안상 등록된 값은 조회 불가(이름만 확인 가능). 값이 필요하면 서버 `.env`에서 확인하거나, 발급처(AWS/OpenAI/LiveKit)에서 재발급.
  ```bash
  gh secret list --repo mjwoon/HYEnd_website   # 이름만 확인
  ```

### 인수인계 완료 체크리스트

- [ ] SSH 키(`hyend-new-key.pem`) 안전하게 전달
- [ ] AWS 콘솔 접근 부여 (계정 공유 또는 IAM 사용자 생성)
- [ ] hosting.kr 도메인 관리 접근 부여
- [ ] GitHub 저장소 collaborator 추가
- [ ] OpenAI / LiveKit 대시보드 접근 부여
- [ ] 새 담당자가 `cat ~/HYEnd_website/.env`로 값 확인 가능한지 검증
- [ ] (권장) 인수인계 후 주요 시크릿 rotation — JWT `SECRET_KEY`, AWS 키 등을 새 값으로 교체하면 이전 담당자 접근을 확실히 차단할 수 있음
